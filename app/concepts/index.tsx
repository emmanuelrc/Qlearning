...
export default function ConceptsIndex() {
  return (
    <View className="flex-1 gap-3 p-6">
      <Text className="text-2xl font-extrabold text-white">Concepts</Text>
      {items.map((i) => (
        <Link key={i.path} href={i.path} asChild>
          <Pressable className="rounded-xl border border-white/10 bg-slate-800/60 p-4">
            <Text className="font-semibold text-white">{i.title}</Text>
          </Pressable>
        </Link>
      ))}
    </View>
